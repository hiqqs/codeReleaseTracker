...
function formatUpdateError(error) {
  ...
  if (
    ...
    /(404|Cannot find|status code 404|HttpError: 404)/i.test(message)
  ) {
    return {
      message: "Unable to check for updates right now.",
      detail:
        "The GitHub release is missing the Windows update manifest (`latest.yml`). Publish the release assets generated by electron-builder, then try again.",
    };
  }
...
async function checkForUpdates(manual = false) {
  ...
  try {
    ...
    if (manual) {
      manualUpdateCheckRequested = false;
      const updateError = formatUpdateError(error);
      await dialog.showMessageBox({
        type: "error",
        buttons: ["OK"],
        defaultId: 0,
